...
<body>
    <div class="wrapper">
        ...
        <div class="content">
		    ...
		    <div class="container">
			<h2 style="text-align:center; font-weight:bold;" class="title mb-1">Listes des Tomes</h2><br>
			<h4 style="text-align:left; font-weight:bold;" class="title mb-1">Volume <?=$_GET['Num_vol'].":".$_GET['titre']?></h4><br>
	<?php
			    ...
			    <div class="item mb-5">
				    <div class="media">
					     <div class="media-body">
						    ...
						    <h3 class="title mb-1"><a href="Article_par_tome.php?tome=<?= $tom['num_tome']?>&titre=<?=utf8_encode($tom['titre_tome'])?>&Num_vol=<?=$_GET['Num_vol']?>&titre_vol=<?=$_GET['titre']?>"><?= "Tome ".$tom['num_tome']." : ".utf8_encode($tom['titre_tome'])  ?></a></h3>
							...
							<tr>		<?php
								  for($i=1;$i<=$pagesTotales;$i++) {?>
									<?php if($i == $pageCourante) {?>
										<?= " <td>".$i."</td> " ?>
									<?php } else {?>
										  <?= '<td><a class="page-link" href="Tomes.php?page='.$i.'">'.$i.'</a></td>'?>
										 <?php   }
										   ?>
								 <?php }
								  ?>
