...
<div class="row">
  <div class="leftcolumn">
	<div class="en_tete_bloc_left"><h3 style="  background: linear-gradient(to right, #c4a218, white);color:white;">Liste des volumes : Textes et documents du cameroun (1815-2012)</h3>  </div>
...
<div class="card1">
      ...
      function sendData<?= $vol['Num_vol'] ?>()
{
	
  
  var titre = document.getElementById("titre<?= $vol['Num_vol'] ?>").value ;
  var prix = document.getElementById("prix<?= $vol['Num_vol'] ?>").value;
  var id_user = document.getElementById("id_user<?= $vol['Num_vol'] ?>").value;
  ...
  return false;
       ...
       <ul class="a">
		<li>  <h4><a style="color:grey;" href="contenu_publication_tomes.php?Num_vol=<?=$vol['Num_vol'] ?>&titre=<?= utf8_encode($vol['Titre_vol']) ?>"><?= "Volume ".$vol['Num_vol']." : ".utf8_encode($vol['Titre_vol'])  ?></a></h4></li>
		</ul><div class="img-magnifier-container"><img id="myimage<?= $vol['Num_vol'] ?>" src="img/volumes/<?= $vol['Num_vol'] ?>.jpg" style="border:1px solid #c4a218; width:200px;height:250px;margin-right:10px; float:left;">
                               </div>
                              <form method="POST" onsubmit="return sendData<?= $vol['Num_vol'] ?>()">
								 <input type="text" name="titre" value="<?= "Volume ".$vol['Num_vol']." : ".utf8_encode($vol['Titre_vol']) ?>" id="titre<?= $vol['Num_vol']?>" hidden >
								 <input type="number" name="prix" value="1500" id="prix<?=$vol['Num_vol']?>"  hidden >
								 <input type="text" name="id_user" value="<?= $_SERVER['REMOTE_ADDR'] ?>" id="id_user<?= $vol['Num_vol']?>"  hidden >
...
<div class="w3-bar" style="text-align:center;">
  <a href="contenu_publication_volumes.php?page=<?=$pageCourante-1 ?>" class="w3-button">«</a>
  <?php
